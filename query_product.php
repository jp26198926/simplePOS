<?php
    ...
    $sql = "SELECT  p.id as id, p.product_code product_code, p.product_name product_name,
                    ...
                    s.status as status,
                    (select sum(qty) from pos_stock where (product_id=p.id AND status_id=1) group by product_id) as stock,
                    (select sum(qty) from pos_sale where (product_id=p.id AND status_id=1) group by product_id) as sale
            FROM (pos_product p
